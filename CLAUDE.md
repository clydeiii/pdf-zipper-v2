...
- **PR2 (next)** — x.com with durable per-media discovery: multi-video tweets are real (43 in twitter.db), the subject AND its quoted tweet, fallback order yt-dlp anon → harvested `video.twimg.com` URL → X-scoped cookies from the personal jar, and the discovery must not depend on the PDF succeeding.
