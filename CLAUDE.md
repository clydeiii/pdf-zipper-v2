...
- **FT gift-link stripping** (`stripFtShareParams`, host-gated to ft.com): `accessToken`/`token`/`sharetype`/`syn-*` are minted per share; article identity is `/content/<uuid>`. Added 2026-09-06 after a re-shared gift link of an already manually-captured article computed a fresh key, bypassed the manual-source guard, and re-ran the FT wall. Stripped forms seeded for the 6 historical entries.
...
- **Tweet late-video re-check** (`src/feeds/poll-worker.ts`, `feed:media-recheck:karakeep`): Karakeep runs yt-dlp on an x.com bookmark AFTER it appears in the API, and the poll usually wins the race — the tweet was processed as a plain link and the video asset that landed a minute later was never collected (6 tweets in 2 days, found by the coverage audit 2026-09-05). Tweets without an enclosure are parked by GUID; every poll looks each up by id for ≈40 min (`FEED_MEDIA_MAX_RECHECKS`=8) and queues the media job the moment the asset exists (`late_video_asset_collected`). Unlike the YouTube wait, the PDF capture proceeds immediately. Helpers in `src/feeds/media-recheck.ts` are queue-free so tests can import them (importing `monitor.ts` opens Redis and hangs `node --test`).
...
### Anchor-Based Truncation Audit (`src/quality/content-anchors.ts`)
Primary Playwright article captures record `SourceTextChars`, optional JSON-LD `SourceWordCount`, and first/middle/last `ContentAnchors` after lazy-body settle and before privacy/print mutations. The nightly capture auditor checks those sentences against its existing PDF text extraction (preserving line breaks for word-wrap normalization). A missing tail or other anchors is reported; a length ratio below 0.6 is reported only alongside a missing anchor. **Report-only (Discord + JSON): no save-time gate changes, automatic rejection, or re-capture.** The motivating review: "missing a distinctive final paragraph is more informative than a generic 20% length discrepancy" — ratios alone are noisy from PDF extraction and repeated page furniture. Excluded: tweets/Nitter, archive snapshots, pass-through PDFs, and rescues. Calibrate on the host with `npx tsx --env-file=.env scripts/anchor-audit-dryrun.ts --limit 20`; add `--self-check` to test PDFs against synthetic anchors before new captures carry metadata.

### Sticky Table Parts Are Content, Not Chrome (`src/converters/pdf.ts` overlay pass, `chrome-extension/content.js`)
Both capture paths remove every `position: fixed`/`sticky` element before printing (navbars, cookie banners, floating widgets). But sites pin a table's header row and label column with `position: sticky` (`thead th`, `th[scope=row]`, a "pinned" first data column) so they stay visible while the table scrolls — and the same pass deleted them: anthropic.com's benchmark table printed as a grid of bare numbers with no model names, no benchmark names and no Fable 5.1 column (2026-09-05, both the Playwright capture and extension v3.3.4). Sticky elements that are table parts (or inside a `table`), captions, headings, `dt`/`summary` are now returned to normal flow (`position: static !important`) instead of removed; `fixed` elements and other sticky chrome are unchanged. Verified surgical: LessWrong, a Substack post, SCMP and a 14-page Wikipedia table page came out byte-identical in text and page count; the log line reports `returned N sticky table/caption element(s) to flow`. Extension users must reload the unpacked extension to pick up v3.3.5.

### Print-Stylesheet Chrome Rules Need a Content Guard (`src/converters/pdf.ts`)
The injected print stylesheet hides modals/CTAs/paywall prompts with **substring class selectors** (`[class*="modal"]`, `[class*="gate"]`, `[class*="paywall"]`…). Those match the class *attribute*, and Tailwind arbitrary variants put whole selectors inside class names — axios.com's story container carries `[&:has(#piano-container…)~.gated-content]:hidden` and `[&_#paywall-fallback-container]:relative`, so two of those rules hid the entire article body and a 921-char headline+hero shell saved as "successful" (2026-09-03). No selector tweak survives the next such class; instead a **content guard** runs after injection: a matched element that is hidden yet holds ≥500 chars and ≥30% of the page's pre-injection text is restored with inline `!important` — **only if it also reads like an article**: ≥3 paragraphs of ≥120 chars, no form controls inside (`input/select/textarea/switch/checkbox/form`), no consent/subscribe/poll class markers, and never a `<script>`/`<style>`. Text volume alone is not enough: thenewstack.io's hidden subscribe modal holds 46% of the page text and its CookieYes modal 75%. Its hidden ancestors are restored with it (they were visible before injection, so our rules hid them — axios's story wrapper contains a newsletter form and fails the widget tests on its own). `CONTENT_GUARDED_HIDE_SELECTORS` mirrors the CSS groups — keep them in sync. Verified: Axios 921→5,065 chars; The New Stack (modals) restores nothing; Dwarkesh/Ars controls byte-identical. The log line names what was restored. **`CAPTURE_PHASE_TRACE=1`** logs body text length at each in-page phase boundary (navigation → scroll/privacy → overlay removal → un-pin → pre-print) — the way this was bisected; run a one-off `convertUrlToPDF` in the container with it set rather than redeploying.
...
**Substack preview gate on the PRIMARY path too** (`src/quality/substack-preview.ts`, worker, 2026-09-05): a paid post rendered without the subscription — or a free post behind Substack's sign-in gate — prints the free portion ending on a clean sentence, our stylesheet hides the "for paid subscribers" box, and the PDF clears every text gate (aidisruption.ai archived 270 of 1,346 words with QualityScore 95). After `analyzePdfContent` passes, the worker fetches the post API's `audience` + true `wordcount` and fails as `paywall:` when extraction is under 90% (paid audiences) or under 50% (`everyone` — a free post's extraction legitimately drops captions/embeds, so only a gross shortfall counts; sources.news rendered 120 of 1,314 words). API failure is never a verdict. The module lives inside the self-heal boundary on purpose: the fidelity corpus replays it via `substackPost` facts, so a batch that loosens it fails the gate. smry's rescue shares the same helper.
...
### Video Acquisition — pdf-zipper downloads every video itself (`src/media/routing.ts`, `src/media/ytdlp-video.ts`)
Owner decision 2026-09-05: Karakeep is the bookmarking service only; the ONE thing still fetched from Karakeep is a PDF the user uploads directly (no other copy exists). Why: Karakeep's asset endpoint served truncated files while its yt-dlp was still writing (21 corrupt MP4s), tweets whose asset landed after the first poll never got a video, YouTube waited ~2h for an asset we then downloaded ourselves, and its bundled yt-dlp was 403-blocked for days in August. Its downloader has no authentication advantage over ours (Astra design review, session scratchpad `astra-media-design.md`).
Staged rollout, per platform:
- **PR1 (shipped 2026-09-05)** — `MEDIA_SOURCE_YOUTUBE=native` (compose; code default `karakeep`): the poll worker gives every YouTube/Vimeo link a `downloadVia: 'yt-dlp'` enclosure immediately (`applyMediaRouting`), ignoring any Karakeep asset, so the metadata worker queues the download on the same poll. The old asset-wait loop (`feed:video-retries:karakeep`) still drains its existing entries, routed natively.
- **PR2 (shipped 2026-09-05)** — `MEDIA_SOURCE_X=native`: EVERY tweet gets a speculative yt-dlp enclosure at poll time (`applyMediaRouting`), so video discovery is independent of the PDF capture and of Karakeep; most tweets settle as `no_media` in one ~1s anonymous extraction. `src/media/x-media.ts` runs the tiers: yt-dlp anonymous (returns all videos of a multi-video tweet) → the harvested `video.twimg.com` URLs from twitter.db (bytes already located, no account; only present once the PDF's harvest ran — the queue's backoff comes back) → yt-dlp with the X-SCOPED cookie subset (`src/media/x-cookies.ts` derives `.cookies-x.txt` from the personal jar so no other site's session, and never the personal Google session, is presented). Multi-video tweets publish `<base>.mp4`, `<base>-2.mp4`, … (each gated, deduped, compressed, enriched). **`LinkedMedia`** (`src/media/pdf-media-link.ts`): the post's PDF lists every MP4 basename acquired for it, including a dedup'd video living under another tweet's name; the media worker and the conversion worker hand off through Redis `media:linked:<key>` so whichever finishes last completes the link (owner request). Media queue keeps 2000 completed jobs so the 3-day coverage audit sees every tweet's outcome.
- **PR3 (flag flipped 2026-09-05)** — `CRAWLER_VIDEO_DOWNLOAD=false` in `~/karakeep/.env` (backup `.env.bak-20260905-pre-native-media`; Karakeep restarted healthy). Karakeep now crawls nothing but page metadata; uploaded PDF assets are still fetched. The legacy code paths (Karakeep video-asset enclosure in `buildKarakeepItem`, the YouTube asset-wait loop, `media-recheck.ts`) are KEPT behind `MEDIA_SOURCE_*=karakeep` as the rollback story for one week — rollback = set both flags to `karakeep`, flip Karakeep's crawler back on, redeploy; never clear the dedup sets. Remove the legacy branches after ~2026-09-12 once a week of coverage audits is clean.
Downloader rules (each one is a scar): ONE format policy `-S res:<VIDEO_COMPRESS_MAX_HEIGHT>,+size` (largest rendition whose SHORTER side ≤ cap — the compressor's rule; the old `/best` fallback escaped the cap); per-attempt staging dir, ffprobe + provenance stamp, atomic rename, staging always removed; `--abort-on-unavailable-fragments` (default yt-dlp silently skips fragments and produces a readable-but-incomplete file); `--max-filesize MEDIA_DOWNLOAD_MAX_MB` plus a staging-size check for HLS; the subprocess runs in its own process group and the GROUP is killed on timeout. **Structured outcomes** (`classifyYtDlpFailure`, tested on real yt-dlp messages): `no_media | unavailable | auth_required | unsupported | policy_exceeded` complete the job and are recorded in the return value (`media_outcome` log event; the coverage audit reads them — `no_media` is a documented skip, the rest are `failed`/`partial` with a cause); `transient | unknown` retry with the queue's backoff. "Terminal" must never be inferred from a vague message — `Video unavailable` and `Unsupported URL` used to complete jobs as "no video". Only ONE re-encode runs at a time (`maybeCompressVideo` mutex, `-threads 2`): two x264 encoders plus two Playwright renderers starved the 4-core host.
...
### YouTube Audio Track Selection — Original Before Small (`src/media/ytdlp-video.ts`)
YouTube now attaches 15+ AI-dubbed audio tracks (`dubbed-auto`) to popular videos. The yt-dlp format sort is `-S res:<cap>,lang,+size` and **`lang` must stay ahead of `+size`**: yt-dlp's language preference ranks the original track above every dub, but with size first the smallest audio track won — a week of captures (≈Sep 1–8, 2026) came through in Malayalam and other dubs (`xOi5nDH0lu0`: old sort → `249-20 Malayalam, low`; fixed → `139-20 English (US) original (default)`). The container's audio `language` tag said `eng` regardless, so tags can't detect a dubbed file; ask yt-dlp which track the old sort picks (`--print "%(language)s|%(format_note)s"`) — that's how the affected files were found and recaptured (mp4 + `.transcript.pdf` deleted, media job re-queued with a fresh jobId). The transcript of a dubbed capture is a transcript of the dub, so recapture must redo enrichment too.

### Duplicate Embedded Videos (`src/media/video-dedup.ts`)
Bookmarking an original tweet AND a quote-tweet of it delivers the same embedded video twice (different Karakeep assets, near-identical bytes); so does bookmarking the same talk from YouTube and from a tweet that re-posted it. Before compressing a fresh download, the collection worker probes the whole library: **VIDEO-STREAM duration** ±500ms + aspect ratio ±1% + same has-audio bit, confirmed by 8x8 perceptual frame hashes at three shared timestamps (≥2 of 3 within 12/64 bits). Never compare the CONTAINER duration: it is the longest track, the re-upload's audio drifts by tens of ms, and `enrichVideo` embeds a subtitle track whose last cue can end seconds after the picture — on 2026-09-06 the same 12:38 talk from YouTube and from x.com had byte-identical video streams (757.880s, 18,947 frames) but container durations of 758.001s vs 763.401s, and the old 50ms container gate stored both. The wider gate is safe only because three frames confirm instead of one (`test/video-dedup.test.js`). A confirmed duplicate is deleted and the new bookmark's URL is appended to the canonical mp4's `also_bookmarked_as` metadata tag (`; `-separated). No state beyond the files themselves — identity is re-derived from probes each time. Dedup is skipped on re-enrich jobs (`existingFilePath` set), which would otherwise match and delete the very file being refreshed. **KB consumers must treat `source_url` + `also_bookmarked_as` as the complete set of tweets referencing a video**; the quote-tweet's own PDF capture carries the quote relationship from the other side. Canonical attribution follows bookmark order (first bookmark wins the filename), so a QT bookmarked before the original keeps the QT's account in the filename — the tags, not the filename, are authoritative.

### Video Dedup Cross-Reference Must Survive Later Writes (`src/metadata/video-tags.ts`)
`appendVideoCrossRef` runs from the DUPLICATE's job and tags the canonical mp4 while the canonical's own job may still be transcribing; every later metadata write (`enrichVideoFile`) now reads and carries an existing `also_bookmarked_as` forward. Before 2026-09-05, 2 of 31 recent dedups had lost the tag — and the KB contract says `source_url` + `also_bookmarked_as` is the complete set of tweets sharing a video.

### Substack pub→host Mapping Persists in Redis
`substack:pubhost` (wired via `setPubHostStore` in `index.ts`) — the in-memory map was wiped by every deploy, each restart cost one HEAD per publication, and Substack 429s a burst of them (which left every iOS-shared post from an unresolved pub "unaccounted" in the coverage audit). The audit additionally matches long `/p/<slug>` paths by slug as the weakest match kind.
...
- **Fidelity gate** (`buildGate` in `fix.worker.ts` → `runFidelityGate()` in `src/quality/fidelity-harness.ts`): after the build, every REVIEWED entry of `{DATA_DIR}/fidelity-corpus/manifest.json` is run through `analyzePdfContent` with its recorded options; one falseAccept (an expected-reject that now passes) fails the batch before a human sees it. Seed candidates with `npm run fidelity-seed` (failed-job debug PDFs + vision-scored good captures, all `reviewed:false`), review, flip `reviewed:true`. The harness, corpus, boundary and these tests are outside the fix write boundary. An empty/absent corpus passes (nothing to judge) — the gate only has teeth once entries are reviewed. **All 40 entries were reviewed 2026-09-05** (Claude + Astra cross-check; every note records the evidence): 12 rejects — 5 Axios `shell`s, 5 `paywall_preview`s (Substack ×4, The Information), the cursor.com `landing_page` (genuinely ~200 chars live: rejecting is correct, don't lower the floor) and the xbow `embedded_viewer` (must stay a reject or the embedded-PDF pass-through never fires) — and 28 accepts, including the Wired capture whose inline "exclusive to subscribers / Log In" box sits above a complete body (an over-eager paywall regex must not reject it). Entries for Substack posts carry `substackPost: {audience, wordcount}` (the post API's facts, recorded by the seeder) and the harness replays `substackPreviewShortfall` on them — that is how the corpus holds the preview gate below: `reject-d373e178…` (aidisruption.ai) was seeded as an ACCEPT with QualityScore 95 and is a confirmed production false accept that `analyzePdfContent` alone still passes. **`expected` encodes what `analyzePdfContent` must RETURN for the file, not whether the capture was faithful** (Astra would accept the cursor.com page as a faithful capture and reject two video tweets whose substance is in the MP4; both dissents are recorded in the notes and overruled on this rule — the MP4 is acquired separately and linked via `LinkedMedia`). The seeder strips Substack share tokens from every `sourceUrl` (the manifest is persisted data; `r=` must not appear in it) and treats retries that differ only in counts ("726 characters" vs "727") as one case.
...
- **Dead URLs are pruned, never diagnosed** (`src/utils/dead-url.ts`, terminal-failure handler, 2026-09-05): a 404 page renders as a ~100-char shell, fails as "truncated", and would be queued for AI repair and then replayed every 12h as a fix "verification" forever (observed: a mis-pathed `anthropic.com/news/…` bookmark; a deleted tweet Nitter reports as 404 but the converter classed `rate_limited`). At terminal failure the origin is probed — tweets via Nitter, since x.com blocks plain fetches — and only a real **404/410** counts as dead (401/403/429/5xx are walls/outages → `unknown`, normal path). Dead URLs skip auto-fix, are recorded with reason `not_found`, and `pruneDeadUrl` removes pending fix items and the Karakeep bookmark(s) behind them (`dead_url_pruned`). User preference: remove these automatically rather than asking.
...
| `MEDIA_SOURCE_YOUTUBE` | `karakeep` (compose: `native`) | Who downloads YouTube/Vimeo: our yt-dlp immediately, or Karakeep's asset (legacy) |
| `MEDIA_SOURCE_X` | `karakeep` (compose: `native`) | Who downloads tweet video: our tiered yt-dlp (anon → harvested CDN → X cookies), or Karakeep's asset + late re-check (legacy) |
| `MEDIA_DOWNLOAD_MAX_MB` | 5000 | Native video download size cap (yt-dlp `--max-filesize` + staging check) |
