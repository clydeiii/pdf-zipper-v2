...
- Route transcription through `transcribeWithRetry()` (same module), never `resolveWhisperHost()` directly. It retries the ASR call up to `TRANSCRIBE_ATTEMPTS` times, re-resolving the host each attempt — a transient primary `/health` blip or fallback 5xx recovers instead of permanently dropping the transcript (video enrichment treats a transcription failure as non-fatal and never revisits it). The `/health` probe also retries once with a generous timeout, so a busy-but-alive primary doesn't trigger needless failover.
