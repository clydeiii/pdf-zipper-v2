...
- **Fidelity gate** (`buildGate` in `fix.worker.ts` → `runFidelityGate()` in `src/quality/fidelity-harness.ts`): after the build, every REVIEWED entry of `{DATA_DIR}/fidelity-corpus/manifest.json` is run through `analyzePdfContent` with its recorded options; one falseAccept (an expected-reject that now passes) fails the batch before a human sees it. Seed candidates with `npm run fidelity-seed` (failed-job debug PDFs + vision-scored good captures, all `reviewed:false`), review, flip `reviewed:true`. The harness, corpus, boundary and these tests are outside the fix write boundary. An empty/absent corpus passes (nothing to judge) — the gate only has teeth once entries are reviewed.
